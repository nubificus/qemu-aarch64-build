#!/bin/bash

#docker run --rm --network=host --device=/dev/net/tun --cap-add NET_ADMIN --gpus all -v $(pwd)/guest/qemu-guest-aarch64:/data -it --device=/dev/kvm nubificus/vaccel-qemu:aarch64
docker run --rm --network=host --device=/dev/net/tun --cap-add NET_ADMIN --gpus all -v $(pwd)/guest/qemu-guest-aarch64:/data --privileged -it nubificus/vaccel-qemu:aarch64
